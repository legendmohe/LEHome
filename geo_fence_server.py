...
class GeoResolver:
    def __init__(self, devices, area):
        self._dev = {}
        self._area = area
    ...
    def resolve(self, target_name):
        ...
        if dev is not None:
            ...
            for area_name, area in self._area.items():
                distance = self.cal_distance(cur_loc.lat, cur_loc.lan, area.lat, area.lan)
    ...
    def dump(self):
        for name, item in self._dev.items():
            print name, [(loc.lat, loc.lan) for loc in list(item.loc_queue)]
...
def load_data_from_conf(path):
    ...
    for name, item in area.items():
        ...
        g_area[name] = Area(name, item["lat"], item["lan"])
...
def start():
    ...
    try:
        g_resolver = GeoResolver(g_devices, g_area)
        while True:
